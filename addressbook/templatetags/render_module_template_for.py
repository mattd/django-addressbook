...
@register.tag
def render_module_template_for(parser, token):
    """
    ...
    If called as ``{% render_module_template_for story_object in "includes/story_list_snippets" %}``
    ...
    return RenderModuleTemplateForNode(object, template_dir[1:-1])
    
class RenderModuleTemplateForNode(template.Node):
